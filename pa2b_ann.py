...
def tune_ann_pytorch(X, y, hidden_layer_sizes, title, k_folds=5, epochs=1000):
        ...
        for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
            ...
            for epoch in range(epochs):
                ...
                for data, target in train_loader:
                    data, target = data.to('cuda' if torch.cuda.is_available() else 'cpu'), target.to(
...
def tune_ann_pytorch_with_early_stopping(X, y, hidden_layer_sizes, title, k_folds=5, epochs=1000, patience=10):
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)
    X_tensor = torch.tensor(X_scaled, dtype=torch.float32)
    y_tensor = torch.tensor(y, dtype=torch.long)
...
tune_ann_pytorch_with_early_stopping(X_higher_ed, y_higher_ed, hidden_layer_sizes, "Higher Education Subset")
